...
// --- NEW HELPER FOR PHASE 4: Handle STREAM operation ---
void handle_stream_from_ss(char* filename, char* ss_ip, int ss_port) {
    ...
    if (ss_header.type == RES_SS_FILE_OK) {
        ...
        char buffer[FILE_BUFFER_SIZE];
        int bytes_recvd;
        // The SS handles the word-by-word logic and delay.
        // The client just prints what it gets as it arrives.
        while ((bytes_recvd = recv(ss_sock, buffer, FILE_BUFFER_SIZE, 0)) > 0) {
            write(STDOUT_FILENO, buffer, bytes_recvd);
...
int main() {
    ...
    // --- 1-6. Registration (Same as Phase 3) ---
    ...
    while (1) {
        ...
        if (strcmp(command, "create") == 0) {
            ...
            if (filename == NULL) { printf("Usage: read <filename>\n"); }
                ...
                if (header.type == RES_READ_LOCATION) {
                    Msg_Read_Response resp; recv(sock, &resp, sizeof(resp), 0);
        ...
        // --- NEW FOR PHASE 4 ---
            ...
            recv(sock, &header, sizeof(header), 0); // Get list header
