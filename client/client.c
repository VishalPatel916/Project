...
int main() {
    ...
    // Connect to localhost. Change "127.0.0.1" to NM's IP if it's on another machine.
    ...
    // 7. Close and exit (for now)
    // In the real project, you would stay connected and enter a command loop
    close(sock);
    return 0;
}